...
func _verify() -> void:
	...
	var game: Node = load("res://scenes/environment/legacy_meadow_game.tscn").instantiate()
	root.add_child(game)
	...
	print("PASS: all 50 atlas slices match originals; 2560 saved ground cells; grass/soil painting and repainting; connected paths; legacy art playground loads terrain.")
	game.free()
	paint.free()
	level.free()
	quit()
